...
## Feature Engineering
Several feature engineering techniques were applied to prepare the data for analysis and modeling. Timestamp columns were converted into datetime format, and additional numerical features such as release year and release month were extracted from time-related columns. Categorical variables were encoded into machine-readable formats, and the target variable, game price (mat_initial_price), was standardized to USD to ensure pricing consistency across currencies. Redundant or uninformative columns were removed, and the datasets were merged into a final integrated dataframe for analysis.
